...
import ChefTableItem from "../../components/admin/ChefTableItem";
...
const Chefs = () => {
  ...
  const filteredService = chefs.filter((chef) => {
    if (filter === "actived") return chef.is_actived;
    if (filter === "unActived") return !chef.is_actived;
    return true;
  });
  ...
  return (
    <div className="relative flex-1 p-6 bg-gray-50 min-h-screen">
      ...
      <div className="w-full flex items-center justify-between">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900">Les chefs cusiniers</h1>
          <p className="text-gray-600 mt-1">Aperçu de tous les chefs cusiniers du site</p>
        </div>
      ...
      <div className="relative h-4/5 w-full overflow-x-auto pt-4 mt-8 bg-white shadow rounded-lg scrollbar-hide">
        <table className="w-full text-sm text-gray-500">
          ...
          <tbody>
            {filteredService.map((chef, index) => (
              <ChefTableItem
                key={index}
                chef={chef}
                index={index + 1}
                toggleChef={toggleChef}
                archiveChef={archiveChef}
                onEdit={() => openEditForm(chef)}
              />
            ))}
          </tbody>
        </table>
